...
void TrafficGraphWidget::saveData()
{
    ...
    try {
        ...
        if (!file) {
            LogWarning("TrafficGraphWidget: Failed to open file for writing: %s\n", pathTrafficGraph.generic_string());
        ...
        LogInfo("TrafficGraphWidget: Successfully saved traffic graph data to %s\n", pathTrafficGraph.generic_string());
    } catch (const std::exception& e) {
        LogWarning("TrafficGraphWidget: Error saving data: %s (path: %s)\n",
                 e.what(), m_data_dir);
    }
}
...
bool TrafficGraphWidget::loadDataFromBinary()
{
    try {
        fs::path pathTrafficGraph = fs::path(m_data_dir.c_str()) / "trafficgraph.dat";
        LogInfo("TrafficGraphWidget: Attempting to load data from %s\n", pathTrafficGraph.generic_string());

        FILE* file = fsbridge::fopen(pathTrafficGraph, "rb");
        if (!file) {
            LogInfo("TrafficGraphWidget: File not found or could not be opened\n");
            return false;
        }
        ...
        LogWarning("TrafficGraphWidget: Error loading data: %s\n", e.what());
        return false;
    }
}
...
bool TrafficGraphWidget::loadData()
{
    ...
    if (!success) { // Zero the values
        LogWarning("TrafficGraphWidget: Saved traffic data was invalid.\n");
