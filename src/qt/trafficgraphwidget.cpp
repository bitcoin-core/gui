...
void TrafficGraphWidget::updateStuff() {
    ...
    uint64_t expected_gap = m_timer->interval();
    uint64_t now = GetTime<std::chrono::milliseconds>().count();
...
bool TrafficGraphWidget::loadDataFromBinary() {
    try {
        ...
        uint64_t totalBytesRecv, totalBytesSent;
        filein >> VARINT(totalBytesRecv) >> VARINT(totalBytesSent);
        m_total_bytes_recv = totalBytesRecv;
        m_total_bytes_sent = totalBytesSent;
